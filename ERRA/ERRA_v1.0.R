...
# Switching to a processor-optimized BLAS, such as the Intel MKL oneAPI Math Kernel Library (or MKL), in the case of computers running Intel processors, can speed up matrix operations by an 
  ...
  function(n , np=1, wt=rep(1, n), split_params = NULL, p_label) {
    ...
    for (ii in 1:np) {  # step through each precipitation series in order
      ...
      for (i in 1:n_crit) {   # step through each criterion in order
        ...
        if (pct_breakpts[[i]]==FALSE) {   # check that breakpoints values are not out of range
...
ERRA <-
...

  {
    ...
    # robust          flag controlling whether robust estimation by Iteratively Reweighted Least Squares (IRLS) will be used.  Note that robust estimation will fail
    #                          if more than half of the values of the response variable (i.e., discharge) are the same (for example, in ephemeral streams with frequent
    #                          discharges of zero).
    ...
    if ((!is.null(xknot_type))&&(!is.null(xknots))) {         # skip all this if either xknot_type or xknots is NULL
      if (substr(xknot_type,1,1)=="v") pct_xknots=FALSE       # xknot_type=="values"
        ...
        na_flag <- as.integer(is.na(rowSums(p_subsets, na.rm=FALSE)) | is.na(q) | !Qfilter)       # boolean vector of rows where any of p_subsets or q is missing
        if (sum(na_flag)>0) {
          if (is.null(h)) missing.cancels <- rep(1, 1+m+max.AR)                        # this is a mask that propagates the effects of missing values forward (assuming maximum possible lag due to AR correction)
          else missing.cancels <- rep(1, 1+m+h)                                        # this is a mask that propagates the effects of missing values forward (assuming fixed h value)
          na_flag <- as.vector(stats::filter(na_flag, missing.cancels, sides=1, circular=TRUE))  # propagate the mask
        }
        ...
        for (j in 1:NCOL(p_subsets)) {
          ...
          xx <- ifelse(na_flag>0, NA, xx)  # remove values that will eventually correspond to missing rows in the analysis
          ...
          for (i in 1:nknots) {
            xx_mincount <- xx[mincount]    # count down from the highest value and save that
            xx_threshold <- xx[1]*(nknots-i+1)/(nknots-i+2)  # calculate the fraction of the highest p value that would correspond to this knot if knots were equally spaced
            
            xx_threshold <- min(xx_mincount, xx_threshold)*0.9999 # pick the higher of these two 
